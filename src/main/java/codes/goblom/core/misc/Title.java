...
@Data
public class Title {
    private static final SafeClass PacketPlayOutTitle = GoPlugin.getInstance().getReflection().getNMSClass("PacketPlayOutTitle");
    private static final SafeClass CraftPlayer = GoPlugin.getInstance().getReflection().getCraftClass("entity.CraftPlayer");
    private static final SafeClass CraftChatMessage = GoPlugin.getInstance().getReflection().getCraftClass("util.CraftChatMessage");
    private static final SafeClass EnumTitleAction = GoPlugin.getInstance().getReflection().getNMSClass("EnumTitleAction");
    private static final SafeClass IChatBaseComponent = GoPlugin.getInstance().getReflection().getNMSClass("IChatBaseComponent");
    ...
    public void send(Player... players) {
        ...
        for (Player player : players) {
            ...
            SafeMethod sendPacket = connection.getSafeClass().getMethod("sendPacket", GoPlugin.getInstance().getReflection().getNMSClass("Packet").unsafe2());
