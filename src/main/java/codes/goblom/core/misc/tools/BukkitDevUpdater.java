package codes.goblom.core.misc.tools;
...
public class BukkitDevUpdater {
    ...
    private final Callback<BukkitDevUpdater> callback;
    // Project's Curse ID
    private int id = -1;
    ...
    private BukkitDevUpdater.UpdateResult result = BukkitDevUpdater.UpdateResult.SUCCESS;
     ...
     * updates in console.
     */
    public BukkitDevUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
     ...
     * finished
     */
    public BukkitDevUpdater(Plugin plugin, int id, File file, UpdateType type, Callback<BukkitDevUpdater> callback) {
     ...
     * updates in console.
     */
    public BukkitDevUpdater(Plugin plugin, int id, File file, UpdateType type, Callback<BukkitDevUpdater> callback, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(BukkitDevUpdater.HOST + BukkitDevUpdater.QUERY + this.id);
     ...
     * @see UpdateResult
     */
    public BukkitDevUpdater.UpdateResult getResult() {
        this.waitForThread();
        return this.result;
    }
    ...
    private void downloadFile() {
        ...
        try {
            ...
            final byte[] data = new byte[BukkitDevUpdater.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, BukkitDevUpdater.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = BukkitDevUpdater.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    int b;
                    final byte[] buffer = new byte[BukkitDevUpdater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, BukkitDevUpdater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, BukkitDevUpdater.BYTE_SIZE)) != -1) {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length == 2) {
                ...
                if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                    ...
                    this.result = BukkitDevUpdater.UpdateResult.NO_UPDATE;
                    return false;
                }
            } else {
                ...
                this.result = BukkitDevUpdater.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean read() {
        try {
            ...
            this.versionName = (String) latestUpdate.get(BukkitDevUpdater.TITLE_VALUE);
            this.versionLink = (String) latestUpdate.get(BukkitDevUpdater.LINK_VALUE);
            this.versionType = (String) latestUpdate.get(BukkitDevUpdater.TYPE_VALUE);
    ...
    private void runCallback() {
        this.callback.onFinish(this, null);
    }
}
